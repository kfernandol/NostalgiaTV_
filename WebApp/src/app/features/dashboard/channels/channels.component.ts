...
export class ChannelsComponent implements OnInit, AfterViewInit {
    ...
    openForm(channel?: ChannelResponse) {
        ...
        const dialogRef = this.dialog.open(GenericFormDialogComponent, { width: '500px', data: config });
    ...
    assignSeries(channel: ChannelResponse) {
        const config: DialogConfig = {
            title: 'Assign Series',
            fields: [{ key: 'seriesIds', label: 'Series', type: 'multiselect', options: this.series().map(s => ({ value: s.id, label: s.name })) }],
